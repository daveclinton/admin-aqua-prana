...
import { teamColumns } from "@/features/team/tables/team-columns"
...
const teamTableSearchParams = createTableSearchParams({
  defaultPageSize: 10,
...
export function TeamTableClient() {
  const [queryState, setQueryState] = useQueryStates(
    teamTableSearchParams.parsers,
    {
      history: "replace",
      urlKeys: teamTableSearchParams.urlKeys,
    }
  )
  ...
  const sorting = useMemo(
    () => parseSortingState(queryState.sort, "lastActiveAt"),
    [queryState.sort]
  )
  ...
  const table = useDataTable<TeamMemberRow>({
    data: rows,
    columns: teamColumns,
    state: tableState,
    totalRows: rowCount,
    ...
    onSortingChange: (updater) => {
      ...
      void setQueryState({
        pageIndex: 0,
        sort: getSortingValue(nextSorting, "lastActiveAt.desc"),
      })
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border bg-card shadow-xs shadow-black/5">
        <DataTableToolbar
          table={table}
          searchPlaceholder="Search team members, roles, or regions"
        />
        <DataTable
          table={table}
